...
if (fs.existsSync(initPath+'/config/config.json')) {
	// http.get('http://bot.whatismyipaddress.com', function(res){
		...
		// res.on('data', function(chunk){
			// fs.writeFile(initPath+'/config/ip', chunk, 'utf8', function (err) {
				// if (err) { };
			// });
		// });
	// });
	// fs.readFile(initPath+'/config/ip', 'utf8', function (err,data) {
		// if (err) {
			// console.log(err);
		// }
		// return data;
...
fs.readFile(initPath+'/config/ip', 'utf8', function readFileCallback(err, data){
    if (err){
        console.log(err);
    } else {
    obj = JSON.parse(data); //now it an object
	console.log(obj);
    obj.host.push({ip: "ip"}); //add some data
    json = JSON.stringify(obj); //convert it back to json
    fs.writeFile(initPath+'/config/ip', json, 'utf8', callback); // write it back 
}});
